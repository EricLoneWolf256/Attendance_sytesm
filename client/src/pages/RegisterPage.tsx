...
export function RegisterPage() {
  ...
  useEffect(() => {
    api
      .get("/campuses")
      .then((res) => setCampuses(res.data.campuses || res.data))
      .catch(() => {});
  }, []);

  const isStudentEmail = email.endsWith("@stud.umu.ac.ug");
  const isStaffEmail = email.endsWith("@umu.ac.ug");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!email.endsWith("@umu.ac.ug")) {
      setError("Please use a valid UMU email address (@umu.ac.ug or @stud.umu.ac.ug)");
      return;
    }
  ...
  const handleGoogleRegister = () => {
    setError("Google SSO is not yet configured. Please use email registration.");
  };

  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 via-white to-gray-50 px-4 py-8">
      ...
      <div className="relative w-full max-w-[480px]">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-1.5 text-sm text-gray-500">
            Use your official UMU email to register
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="block w-full rounded-lg border border-gray-200 bg-gray-50/50 px-3.5 py-2.5 text-sm text-gray-900 transition-all placeholder:text-gray-400 hover:border-gray-300 focus:border-umu-red focus:bg-white focus:outline-none focus:ring-2 focus:ring-umu-red/10"
              />
              {email && !isStaffEmail && !isStudentEmail && (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="block w-full rounded-lg border border-gray-200 bg-gray-50/50 px-3.5 py-2.5 text-sm text-gray-900 transition-all hover:border-gray-300 focus:border-umu-red focus:bg-white focus:outline-none focus:ring-2 focus:ring-umu-red/10"
                >
              ...
              <div>
                <label htmlFor="campus" className="mb-1.5 block text-sm font-medium text-gray-700">
                  Campus
                </label>
                <select
                  id="campus"
            ...
            <button
              type="submit"
              disabled={isSubmitting || !campusId}
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          ...
          <Link to="/login" className="font-semibold text-umu-red transition-colors hover:text-umu-red-dark">
            Sign in
          </Link>
        </p>
